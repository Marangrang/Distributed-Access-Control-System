...
METADATA_PATH = os.getenv(
    "METADATA_PATH",
    "verification_service/faiss_index/metadata.json")
...
try:
    if os.path.exists(INDEX_PATH):
        ...
        logger.warning(
            "FAISS index not found at %s — creating empty IndexFlatIP(512).",
            INDEX_PATH)
...
def embed_image_from_base64(b64: str):
    ...
    if face is None:
        ...
        face = torch.tensor(
            np.array(img_resized).transpose(
                2, 0, 1)).float() / 255.0
    face = face.to(device).unsqueeze(
        0) if face.ndim == 3 else face.unsqueeze(0)
    with torch.no_grad():
...
def verify(payload: VerifyPayload):
    ...
    match = len(
        candidates) > 0 and candidates[0]["score"] >= SIMILARITY_THRESHOLD
...
# NOTE: All models, index, and metadata are preloaded at startup for low
# latency. No per-request disk I/O.
